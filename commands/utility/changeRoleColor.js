...
module.exports = {
    ...
    run: async(client, message, args) => {
       ...
       if(!role) {
           ...
           .setDescription(`${emojis.error} Please specify a role.`)
           .setColor("#FF0000")
     ...
     if(!message.member.permissions.has("MANAGE_ROLES")) {
      ...
      .setDescription(`${emojis.error} You don't have the following permission: \`Manage Roles\``)
      .setColor("#FF0000")
     ...
     if(!message.guild.me.hasPermission('MANAGE_ROLES')) {
         ...
         .setDescription(`${emojis.error} The command couldn't run because I don't have the permission to \`Manage Roles\`.`)
         .setColor("#FF0000")
    ...
    // Embed sent if the color isn't specified
     if (!color) {
         ...
         .setDescription(`${emojis.error} Please specify a color for me to change. You can say a [hex color code](https://htmlcolorcodes.com).\n(Example: \`#FF0000\`)`)
         .setColor('#FF0000')
    ...
    try {
        ...
        .setDescription(`${emojis.success} Successfully changed ${role}'s color to \`${color}\`.`)
        .setColor(color) 
        ...
        .setTitle('Error')
        .setDescription(`${emojis.error} Please hoist my role above the role you mentioned so I can change the color.`)
