...
const ms = require('ms');
const { emojis, footer, color } = require('../../config/bot')
...
.setDescription(`${emojis.success} The slowmode has been set to ${ms(milliseconds, { long: true })}.`)
.setColor(color.success)
}

catch(error) {
    const embed = new MessageEmbed()
    .setDescription(`${emojis.error} An error had occured:\n \`\`\`${error}\`\`\``)
