...
const { filters, emojis, color, footer, discord } = require('../../config/bot');
...
if (!message.member.voice.channel) {
  ...
  if (client.player.getQueue(message).paused) { 
      ...
      .setDescription(`${emojis.error} The music is already paused.`)
      ...
      return message.channel.send(musicAlreadyPaused)
        ...
        if (success) {
            ...
            .setDescription(`${emojis.success} The song **${client.player.getQueue(message).playing.title}** has been paused.`)
            .setFooter(`Do ${discord.prefix}resume to resume the song(s).`)
